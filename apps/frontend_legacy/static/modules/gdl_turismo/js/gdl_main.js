document.addEventListener("DOMContentLoaded", async function () {
  const gdlConfig = window.GDL_CONFIG || {};
  const apiColoniasUrl = gdlConfig.apiColonias || "/api/gdl/colonias";
  const apiRutaUrl = gdlConfig.apiRuta || "/api/gdl/ruta";
  const puntosTuristicosUrl = gdlConfig.puntosTuristicosUrl || "/gdl_static/data/gdl_puntos_turisticos_ruta.json";
  ...
  async function cargarZonasTuristicas() {
    ...
    try {
      const response = await fetch(zonasTuristicasUrl);
  ...
  async function cargarPuntosTuristicos() {
    ...
    try {
      const response = await fetch(puntosTuristicosUrl);
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
  ...
  async function cargarColonias() {
    ...
    try {
      const response = await fetch(`${apiColoniasUrl}?municipio=${encodeURIComponent(municipio)}`);
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
  ...
  btnCalcular.addEventListener("click", async function () {
    ...
    try {
      const response = await fetch(apiRutaUrl, {
        method: "POST",
        headers: {
